		...
		switch (event.body.method) {
			case "new":
				...
				if (event.body.studentID != null && event.body.teacherID != null) {
					...
					var teacherID;
					var studentID;
					var selectQueryteacherID = `SELECT  teacherID FROM  nextdoormilwaukeedb.Teachers t where t.nickName='${event.body.teacherNickName}';`;
					var selectQuerystudentID = `SELECT  studentID FROM  nextdoormilwaukeedb.Students s where s.nickName='${event.body.studentNickName}';`;
					var insertQuery2 = `INSERT INTO nextdoormilwaukeedb.logs (teacherID,studentID,activityDetails,activityType,time) VALUES`;
					connection.query(selectQueryteacherID + selectQuerystudentID, function (error, results, fields) {
						if (error) {
							callback(null, {
								"statusCode": 400,
								"body": error
						...
						teacherID = results[0][0].teacherID;
						studentID = results[1][0].studentID;
						insertQuery2 += `(${teacherID},${studentID},'${event.body.activityDetails ? "'" + event.body.activityDetails+ "'" : 'NULL'}','${event.body.activityType}',NOW());`;
						connection.query(insertQuery2, function (error, results, fields) {
							if (error) {
								callback(null, {
									"statusCode": 400,
									"body": error
